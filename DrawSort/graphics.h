...
void message(int x, int y, int number) {
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
	/*
		LPCWSTR numbers[20 + 1] =
		{
			L"0", L"1", L"2",
			L"3", L"4", L"5",
			L"6", L"7", L"8",
	...
	LPCWSTR	message = L"7346374637463894734893748943473894";
...
void outnumberxy(int x, int y, int n)
{
	gotoxy(x / 8, y / 12);
	std::cout << n;
}
...
COORD setCordScale(COORD POS, RECT pRECT)
{
	...
	double CX = (double)(pRECT.right - pRECT.left) / nCOLS;
	double CY = (double)(pRECT.bottom - pRECT.top) / nROWS;
